...
export default async (req: NextApiRequest, res: NextApiResponse<ApplicationData[] | string>): Promise<void> => {
	if (req.method === 'GET') {
		try {
			const client = await clientPromise;
			const db = client.db('main');

			const data = await db.collection<MyMLHUser & { checkedIn: boolean }>('applications').find().toArray();
